...
type Node interface {
	...
	Tick(ctx context.Context, ins []io.Reader, outs []io.Writer)
...
func (graph *Graph) Tick(ctx context.Context) {
	ctx, span := telemetry.Tracer.Start(ctx, "Graph.Tick")
	defer span.End()

	graph.compositeNode.Tick(ctx, []io.Reader{}, []io.Writer{})
}
